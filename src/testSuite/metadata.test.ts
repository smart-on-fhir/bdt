...
suite("Metadata", () => {
    suite("CapabilityStatement", () => {
        ...
        test({
            ...
            try {
                expect(stmt.instantiates.indexOf("http://hl7.org/fhir/uv/bulkdata/CapabilityStatement/bulk-data")).to.be.greaterThan(-1);
            } catch(ex) {
        ...
        test({
            ...
            let extensions;
            try {
                extensions = stmt.rest[0].security.extension.find((e: any) => {
            ...
            const extToken = extensions.find((e: any) => e.url === "token");
        ...
        test({
            ...
            const stmt = await client.getCapabilityStatement();
            try {
                let operation = stmt.rest[0].operation.find((e: any) => e.name === "export");
                if (!operation) {
                    throw new Error();
                }
            } catch (ex) {
        ...
        test({
            ...
            const stmt = await client.getCapabilityStatement();
            try {
                let patient = stmt.rest[0].resource.find((r:any) => r.type.toLowerCase() == "patient");
                let operation = patient.operation.find((e:any) => e.name === "patient-export");
                if (!operation) {
                    throw new Error();
                }
            } catch (ex) {
        ...
        test({
            ...
            const stmt = await client.getCapabilityStatement();
            try {
                let group = stmt.rest[0].resource.find((r: any) => r.type.toLowerCase() == "group");
                let operation = group.operation.find((e: any) => e.name === "group-export");
                if (!operation) {
                    throw new Error();
                }
            } catch (ex) {
    ...
    suite("Well Known SMART Configuration", () => {
        test({
            ...
            try {
                expect(body).to.include("token_endpoint");
            } catch(ex) {
