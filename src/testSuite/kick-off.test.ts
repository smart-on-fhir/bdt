...
suite("Kick-off Endpoint", () => {
    ["patient", "system", "group"].forEach((type: exportType) => {
        suite(`Making a ${type}-level export`, () => {
            ...
            suite("Request Parameters", () => {
                ...
                // _elements ---------------------------------------------------
                ["GET", "POST"].forEach((method: "GET" | "POST") => {        
                    test({
                        ...
                        // resource.
                        expectNDJSONElements(response.body, ["id", `${resourceType}.meta`], "Invalid NDJSON structure")
                ...
                if (type !== "system") {
                    test({
                        ...
                        expect(lines2.length, "Could not filter the results using the patient parameter").to.equal(1);
                        ...
                        expect(patient2.id, `Could not filter the results using the patient parameter. The patient ID should equal "${patient.id}"`).to.equal(patient.id);
                    });
                } else {
                    test({
