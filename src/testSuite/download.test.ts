...
suite("Download Endpoint", () => {
    ...
    test({
        ...
        try {
            ...
            if (body.requiresAccessToken) {
                ...
                expect(response.statusCode, getErrorMessageFromResponse(response)).to.be.above(399);
            } else {
                api.console.md(
                    "This test was not executed because the " +
                    "`requiresAccessToken` field in the complete status " +
                    "body was **not** set to `true`.",
                    "info",
                    "NOTE"
                );
            }
    ...
    test({
        ...
        try {
            ...
            if (!client.statusResponse.body.requiresAccessToken) {
                const response = await client.downloadFileAt(0, true);
                expect(response.statusCode, "Downloading a file without authorization").to.be.below(400);
            } else {
                api.console.md(
                    "This test was not executed because the `requiresAccessToken` field in the complete status body was set to `true`.",
                    "info",
                    "NOTE"
                );
            }
    ...
    test({
        ...
        if (!client.statusResponse.body.requiresAccessToken) {
            ...
            return api.console.md(
                "This test was not executed because the `requiresAccessToken` field in the complete status body was NOT set to `true`.",
                "info",
                "NOTE"
            );
        }
        ...
        expect(resp2.statusCode, `Download should fail if the client does not have proper scopes. ${getErrorMessageFromResponse(resp2)}`).to.be.above(399);
    });
    ...
    test({
        ...
        try {
            ...
            for (let entry of client.statusResponse.body.output) {
                ...
                expectSuccessfulDownload(resp, `Failed to download file from ${entry.url}`)
                    ...
                    for (let item of documentReference.content) {
                        ...
                        if (!urlChecked && item.attachment.url) {
                            ...
                            if (!skipAuth) {
                                ...
                                expect(response.statusCode, `The attachment at ${item.attachment.url} should not be downloadable without authentication`).to.be.above(399)
                                expect([400, 401, 403], `The rejects the download but replies with unexpected status code "${response.statusCode}"`).to.include(response.statusCode)
                            }
                            ...
                            expect([200, 304], `The file at  at ${item.attachment.url} cannot be download`).to.include(response.statusCode)
                        }
                    }
                }
            }
