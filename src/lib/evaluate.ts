...
async function check(name: string, weights: Weights, fn: () => any)
{
    ...
    if (result === true) {
        ...
        if (weights.security   ) _score.security   .log.push({ label: name, pass: false })
        if (weights.compliance ) _score.compliance .log.push({ label: name, pass: false })
        if (weights.reliability) _score.reliability.log.push({ label: name, pass: false })
        if (weights.performance) _score.performance.log.push({ label: name, pass: false })
    }
}

function reset() {
    for (let key in _score) {
        const meta = _score[key as keyof Score];
        meta.current = 0
        meta.total = 0
        meta.log = []
    }
...
async function evaluate() {
    ...
    check(
        "tokenUrl uses https",
        { security: 5, reliability: 3 },
        urlIsHttps(config.authentication?.tokenEndpoint)
    )
    
    check(
        "tokenUrl does not include username",
        { security: 5, reliability: 5 },
        urlHasNoUsername(config.authentication?.tokenEndpoint)
    )

    check(
        "tokenUrl does not include password",
        { security: 5, reliability: 5 },
        urlHasNoPassword(config.authentication?.tokenEndpoint)
    )
    ...
    if (config.authentication?.jwksUrl) {
        check(
            "jwksUrl uses https",
            { security: 5, reliability: 3 },
            urlIsHttps(config.authentication.jwksUrl)
        )
    }

    check(
        "Does not require clients to request write scopes",
        { compliance: 3, security: 5, reliability: 3 },
...
async function report() {
    ...
    for (const criteria in _score) {
        const meta = _score[criteria as keyof Score];
        if (meta.total > 0) {
