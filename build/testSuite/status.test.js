    ...
    bdt_1.test({
        ...
        const { response: kickOffResponse } = await client.kickOff({
            params: {
                ...
                _since: moment_1.default().subtract(1, "day").format("YYYY-MM-DDTHH:mm:ssZ")
            }
        });
        ...
        code_1.expect(client.statusResponse.statusCode, "The status code returned by the status endpoint must be 202").to.equal(202);
    });
    bdt_1.test({
        id: "Status-03",
        ...
        code_1.expect(client.statusResponse.statusCode, "Responds with 200 for completed exports").to.equal(200);
        code_1.expect(client.statusResponse.headers["content-type"], "responds with JSON").to.match(/^application\/json/i);
        ...
        if (client.statusResponse.headers["expires"]) {
            code_1.expect(client.statusResponse.headers["expires"], "the expires header must be a string if present").to.be.a.string();
            // If expires header is present, make sure it is in the future.
            const expires = moment_1.default(client.statusResponse.headers["expires"], [
            ...
            code_1.expect(expires.diff(now, "seconds") > 0, "The expires header of the status response should be a date in the future").to.be.true();
            ...
            if (client.statusResponse.headers["date"]) {
                const date = moment_1.default(client.statusResponse.headers["date"]).utc(true);
                code_1.expect(expires.diff(date, "seconds") > 0, "The expires header of the status response should be a date after the one in the date header").to.be.true();
            }
        }
        ...
        code_1.expect(body, "the response contains 'transactionTime'").to.include("transactionTime");
        code_1.expect(body.transactionTime, "transactionTime must be a string").to.be.a.string();
        code_1.expect(body.transactionTime, "transactionTime must be FHIR instant").to.match(REGEXP_INSTANT);
        // the full URI of the original bulk data kick-off request
        code_1.expect(body, "the response contains 'request'").to.include("request");
        code_1.expect(body.request, "the 'request' property must contain the kick-off URL").to.equal(client.kickOffRequest.options.url.href);
        ...
        code_1.expect(body, "the response contains 'requiresAccessToken'").to.include("request");
        code_1.expect(body.requiresAccessToken, "the 'requiresAccessToken' property must have a boolean value").to.be.boolean();
        ...
        code_1.expect(body, "the response contains an 'output' array").to.include("output");
        code_1.expect(body.output, "the 'output' property must be an array").to.be.an.array();
        body.output.forEach(item => {
            ...
            // in the response.
            code_1.expect(item, "every output item must have 'type' property").to.include("type");
            code_1.expect(item.type, "every output item's 'type' property must equal the exported resource type").to.include(resourceType);
            ...
            code_1.expect(item, "every output item must have 'url' property").to.include("url");
            code_1.expect(item.url, "every output item url must be a string").to.be.a.string();
            ...
            if (item.hasOwnProperty("count")) {
                ...
                code_1.expect(item.count, "if set, output item count must be a number").to.be.a.number();
            }
        });
        ...
        code_1.expect(body, "the response contains an 'error' array").to.include("error");
        code_1.expect(body.output, "the 'error' property must be an array").to.be.an.array();
        body.error.forEach(item => {
            ...
            // data are found for a resource, the server SHOULD NOT return an output item for that resource
            // in the response.
            code_1.expect(item, "every error item must have 'type' property").to.include("error");
            code_1.expect(item.type, "every error item's 'type' property must equal the exported resource type").to.include(resourceType);
            // url - the path to the file. The format of the file SHOULD reflect that requested in the
            // _outputFormat parameter of the initial kick-off request. Note that the files-for-download
            // MAY be served by a file server other than a FHIR-specific server.
            code_1.expect(item, "every error item must have 'url' property").to.include("url");
            code_1.expect(item.url, "every error item url must be a string").to.be.a.string();
            // Each file item MAY optionally contain the following field:
            if (item.hasOwnProperty("count")) {
                // count - the number of resources in the file, represented as a JSON number.
                code_1.expect(item.count, "if set, error item count must be a number").to.be.a.number();
            }
        });
    });
    bdt_1.test({
        ...
        assertions_1.expectSuccessfulKickOff(kickOffResponse, api);
        ...
        code_1.expect(cancelResponse1.statusCode, "Servers that support export job canceling should reply with 202 status code").to.equal(202);
        ...
        code_1.expect(cancelResponse2.statusCode, msg).to.equal(404);
        assertions_1.expectOperationOutcome(cancelResponse2, msg);
    });
    bdt_1.test({
        ...
        const _since = moment_1.default().subtract(1, "day").format("YYYY-MM-DDTHH:mm:ssZ");
        ...
        assertions_1.expectSuccessfulKickOff(response, api, "Kick-off failed");
        await client.waitForExport();
